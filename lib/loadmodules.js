...
var fs = require('fs')
  ...
  , __config = {
      rootPath: 'modules/'
    , globalVar: 'modules'
    , extensions: ['*.js', '.txt#text'] // necessary to pass "#text" when the type will be text
    , ignore: []
  ...
  files.forEach(function(file) {
    ...
    _path.reverse().forEach(function(path, index) {
      if(path !== '.' && path !== '..') {
        ...
        if(index === 0) {
          path = path.split('.')[0];
          objTemp[path] = vm.loadFile(file) /*require(vm.__config.serverPath + file)*/ /*file*//*require('./' + file)*/;
        } else {
          objTemp[path] = obj;
        }
...
LoadModules.prototype.loadFile = function(_filePath) {
  var vm = this
    , ext = vm.__config.extensions.filter(function(ex) { return ex.indexOf(_filePath.slice(_filePath.lastIndexOf('.'), _filePath.length)) >= 0;});

  if(ext.length && ext.filter(function(ex) { return ex.indexOf('#text') >= 0;}).length) {
    return fs.readFileSync('./' + _filePath, {encoding: 'utf-8'});
  } else {
    return require(vm.__config.serverPath + _filePath);
  }
};//loadFile
